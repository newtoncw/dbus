...
DBusTrustedConnection* dbus_bus_get_trusted (DBusBusType type, DBusError *error) {
	...
	_dbus_trusted_connection_set_enclave_id(conn, eid);
...
DBusTrustedSession* dbus_bus_create_trusted_session (DBusTrustedConnection *connection, const char *destination, const char *path, const char *iface, DBusError *error) {
	...
	ret = _dbus_enclave_init_session(_dbus_trusted_connection_get_enclave_id(connection), &sgx_dh_session);
	...
	DBusTrustedSession* session = _dbus_trusted_connection_create_session (connection, destination, path, iface, uid);

	ret = _dbus_enclave_initiator_proc_msg1(_dbus_trusted_connection_get_enclave_id(connection), uid, &dh_msg1, &dh_msg2, &sgx_dh_session);
	...
	ret = _dbus_enclave_initiator_proc_msg3(_dbus_trusted_connection_get_enclave_id(connection), uid, &dh_msg3, &sgx_dh_session);
...
void dbus_bus_close_trusted_session (DBusTrustedSession *session, DBusError *error) {
	...
	sgx_status_t ret = _dbus_enclave_shared_close_session(_dbus_trusted_connection_get_enclave_id_from_session(session), _dbus_trusted_connection_get_session_id(session));
